...
function load(lstContacts) {
    var lstContacts = lstContacts == undefined ? [] : lstContacts;
...
function save(contact_id) {
    ...
    var btnId = '#btnView-' + contact['id'] + ' i';
    if (contact['obs'] != '') {
        $(btnId).removeClass('fa fa-file-text-o');
        $(btnId).addClass('fa fa-file-text');
    } else {
        $(btnId).removeClass('fa fa-file-text');
        $(btnId).addClass('fa fa-file-text-o');
    }
}
